...
const MIN_NIGHTS_OPTIONS = ["1", "2", "3", "5", "7", "14"];
...
function PillButton({ label, active, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-10 h-10 rounded-full text-sm border transition-all flex items-center justify-center ${
        active
                      ...
                      <div className="flex flex-wrap gap-2">
                        {MIN_NIGHTS_OPTIONS.map((n) => (
                          <PillButton
                            ...
                            onClick={() =>
                              set("minNights")(filters.minNights === n ? "" : n)
                            }
                          />
                        ))}
                      </div>
